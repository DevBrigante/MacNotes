...
- `Collapsed` draws in the menu bar beside the notch, and `Expanded` covers that strip with a window even though it draws below it. Either way the clicks under them are ours, not the menu bar's. That strip is the app's surface; this is the cost of having one.
