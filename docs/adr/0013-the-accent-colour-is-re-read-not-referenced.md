...
Issue #9 took the seam. The Planner is an ordinary window that follows light and dark, and there the appearance variance the re-read flattens is the thing that matters: a colour resolved to sRGB once, in whichever appearance happened to be current, would sit wrong in the other one for the life of the window. So the Planner takes SwiftUI's `Color.accentColor` and lets it resolve at draw time. The two surfaces differ because their chrome does — the Panel paints its own black in both appearances and redraws only when its state changes, and an ordinary window is redrawn for it.
