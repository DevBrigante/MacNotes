...
Holding the Mac awake for the length of a Session was the way to make the rule only ever about the lid. `ProcessInfo.beginActivity` with `.idleSystemSleepDisabled` keeps idle sleep off while a Session runs, so an hour spent half on the screen and half on a paper book would come back to a countdown still going. It is declined. It makes MacNotes the reason a machine stays awake — a claim on someone's battery and someone's fans, held for as long as a Session lasts — and it buys a guess: twenty idle minutes are no evidence that the Task was what they went to. The rule is better as one sentence than as a list of the ways a machine can suspend.
...
A Mac that sleeps on its own ends a Session as surely as a closed lid does, and says nothing first. That is the cost of the paragraph above, accepted rather than overlooked.
