...
What is left worth persisting is a Session that is still running when the app goes away, and its value is smaller than it looks. Time passes while the app is gone, and a Session read back at launch has almost always run out during it, so the honest thing to do with it is end it — which is also what ADR-0010 does to a Session the Mac slept through, from the same reasoning — which is what starting with none does, without a file, a schema and a migration. In the minutes where it has not run out, restoring it means restoring a countdown the user did not watch. Completion is their explicit act, so nothing was lost by it either way.
