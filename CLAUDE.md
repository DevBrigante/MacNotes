...
4. **Delete the branch once the PR is merged**, on the remote and locally, so only `main` is left. `git branch -d` refuses a branch that reached `main` by squash or rebase, because it checks ancestry and those land as different commits — confirm with `git diff --stat main <branch>`, which comes back empty once every byte of the branch is on `main`, then `git branch -D`. Not `git cherry`: it matches patch IDs one commit at a time, so a squash of more than one commit reads as `+`, never upstream, however completely it landed. GitHub deletes the remote branch itself on merge; `git fetch --prune` clears the tracking ref left behind.
